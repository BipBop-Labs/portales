...
Every command is one `CommandSpec` in a per-service `ServiceSpec` (`apps/cli/src/commands/<service>.ts`), registered in `apps/cli/src/registry.ts`. The spec declares path, effect, auth, browser requirement, profile policy, arguments with discovery commands, output description, error codes, and the dated contract reference. Hierarchical help, `catalog`, `describe`, argument validation with actionable errors, run ids, lifecycle events, locks, run records, and envelopes are generated by the runner (`apps/cli/src/cli.ts`) from that declaration.
...
## Runs, locks, and artifacts

Each service command gets a `runId`. The runner emits lifecycle events to STDERR, acquires a per-service/profile lock for browser-backed commands (returning `RUN_LOCKED` with the active run id instead of failing ambiguously), and writes a private bounded run record. Downloaded documents are indexed as artifact descriptors (`packages/runtime/src/artifacts.ts`) with SHA-256, byte count, media type, identifiers, validation checks, and private path; `artifacts list|latest|show|verify` read that index so downstream tools never scan directories. Destinations (`packages/runtime/src/destinations.ts`) resolve `--output`, a private alias from `$XDG_CONFIG_HOME/portales/destinations.json`, or the default isolated directory by service, profile, document type, and selected identifiers.

## Contracts

A browser contract is Markdown explanation plus a JSON fingerprint beside it: expected page states as aria snapshots (roles and names, values stripped), control cardinalities, frame names, sanitized route fragments, readiness and stop conditions, and an observation date. Adapters classify the current page against those fingerprints before parsing and report the smallest structural diff on mismatch. Observation mode and `contract check` emit the same format, so a repair is a reviewed diff of one file. Nothing self-modifies at runtime, and no live DOM leaves the machine.
